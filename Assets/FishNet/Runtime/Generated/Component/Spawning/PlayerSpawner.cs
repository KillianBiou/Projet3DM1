...
namespace FishNet.Component.Spawning
{
    ...
    public class PlayerSpawner : MonoBehaviour
    {
        ...
        [SerializeField]
        private NetworkObject _gamemasterPrefab;
        [Tooltip("Prefab to spawn for the game master.")]
        [SerializeField]
        ...
        private void SceneManager_OnClientLoadedStartScenes(NetworkConnection conn, bool asServer)
        {
            if (!asServer)
                return;
            if (_gamemasterPrefab == null)
            {
                Debug.LogWarning($"Player prefab is empty and cannot be spawned for connection {conn.ClientId}.");
                return;
            }
            else if(conn.ClientId > 0)
            {
                _gamemasterPrefab = _playerPrefab;
            }


            Vector3 position;
            Quaternion rotation;
            SetSpawn(_gamemasterPrefab.transform, out position, out rotation);

            NetworkObject nob = _networkManager.GetPooledInstantiated(_gamemasterPrefab, true);
